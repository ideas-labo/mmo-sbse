...
ALGORITHM_DIR_MAP = {
    'NSGA2': os.path.abspath('../../../../Results/RQ1-raw-data/SEE')
}
...
def format_dataset_name(dataset_name):
    is_reverse = False
    if dataset_name.endswith('_reverse'):
        is_reverse = True
        dataset_name = dataset_name[:-8]

    parts = dataset_name.split('_')
    if len(parts) >= 2:
        formatted_name = f"{parts[0]}-train_{parts[1]}"
        if len(parts) > 2:
            for part in parts[2:]:
                formatted_name += f"_{part}"
    else:
        formatted_name = dataset_name

    if is_reverse:
        formatted_name += '_reverse'
...
def read_non_ft_best_nsga2_data(selected_datasets, start_seed, end_seed, output_path=None):
    ...
    for file in os.listdir(nsga2_dir):
        ...
        if dataset_name in MAXIMIZATION_DATASETS_FT and not is_reverse:
            processed_ft = -raw_ft
        else:
            processed_ft = raw_ft
    ...
    for dataset_name in all_data_df['Dataset Name'].unique():
        ...
        try:
            ...
            best_modes_by_dataset[dataset_name] = all_modes_ranking[0]['mode']
                ...
                if not pd.isna(median_val):
                    default_row = {
                        ...
                        'Original Dataset': dataset_data['Original Dataset'].iloc[
                            0] if not dataset_data.empty else dataset_name.replace('_reverse', ''),
                        'Random Seed': seed,
    ...
    if output_path and not complete_ranking_df.empty:
        try:
            ...
            complete_ranking_df_formatted = complete_ranking_df.copy()
            complete_ranking_df_formatted['Dataset Name'] = complete_ranking_df_formatted['Dataset Name'].apply(
                format_dataset_name)

            complete_ranking_df_formatted[['Dataset Name', 'mode', 'unique_rank', 'group_rank', 'is_best']].to_csv(
...
def main():
    config = {
        ...
        'ranking_filename': 'non_ft_modes_ranking_see.csv',
    }

    selected_datasets = []
    dataset_names = ["china", "desharnais", "finnish", "maxwell", "miyazaki"]
    folds = ["fold1", "fold2", "fold3"]
    for dataset in dataset_names:
        for fold in folds:
            selected_datasets.append(dataset + "_" + fold)
