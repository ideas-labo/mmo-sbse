...
def create_ranking_df_from_external_see(selected_datasets_fold, selected_modes, ranking_dict, process_reverse=False):
    """
    Create SEE ranking DataFrame from external ranking information

    Parameters:
        selected_datasets_fold: Selected dataset list with fold information
    ...
    all_datasets_to_process = selected_datasets_fold.copy()
    if process_reverse:
        all_datasets_to_process += [f"{ds}_reverse" for ds in selected_datasets_fold]
    ...
    for dataset_name in all_datasets_to_process:
        ...
        # Extract base dataset name (without fold and reverse)
        base_dataset = dataset_name.replace("_reverse", "") if is_reverse else dataset_name

        for mode in selected_modes:
            # Create key for lookup (using base dataset name without fold)
            ...
            if key in ranking_dict:
                ...
                print(
                    f"[WARNING] No ranking info found for dataset '{dataset_name}' (base: '{base_dataset}'), mode '{mode}', reverse={is_reverse}, using default rank 1")
                ranking_data.append({
                    'Dataset Name': dataset_name,
                    'mode': mode,
                    'is_reverse': is_reverse,
    ...
    if ranking_data:
        ...
        print(f"[INFO] Created ranking DataFrame with {len(ranking_df)} rows for SEE")
        return ranking_df
    else:
        print("[WARNING] Failed to create any SEE ranking information")
        return pd.DataFrame()
...
def coordinated_pipeline_see(
    ...
    # Add fold information to dataset names
    ...
    print(f"Number of datasets with folds: {len(selected_datasets_fold)}")
    ...
    sampling_data_exists = check_sampling_data_exists(selected_datasets_fold, sampling_methods, num_samples,
                                                      random_seeds)
    ...
    multi_feature_data_exists = check_multi_feature_data_exists(selected_datasets_fold, selected_modes,
                                                                sampling_methods)
    ...
    ranking_df = create_ranking_df_from_external_see(selected_datasets_fold, selected_modes,
    ...
    if combined_dfs:
        ...
        output_filename = 'processed_data_see.csv'
        ...
        print("\n" + "=" * 60)
        print("SEE Data processing pipeline completed using external ranking information")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    ranking_csv_path = '../../../Results/Predict-raw-data/Ranking/non_ft_modes_ranking_see.csv'
