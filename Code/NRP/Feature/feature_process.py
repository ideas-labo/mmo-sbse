...
def create_ranking_df_from_external_nrp(selected_datasets, selected_modes, ranking_dict, process_reverse=False):
    """
    Create NRP ranking DataFrame from external ranking information
...
def validate_ranking_coverage(ranking_df, selected_datasets, selected_modes, process_reverse=False):
    ...
    # Count unique combinations with valid ranking (not default rank 1)
...
def coordinated_pipeline_nrp(
    ...
    print("=" * 60)
    print("Starting NRP Coordinated Data Processing Pipeline with External Ranking")
    print("=" * 60)
    ...
    ranking_df = create_ranking_df_from_external_nrp(
    ...
    print("\nStage 4: Data merging and processing (skipping NSGA2 data reading)")
    ...
    print(f"NRP landscape_df shape: {landscape_df.shape}")
    print(f"NRP combined_sampling_df shape: {combined_sampling_df.shape}")
    print(f"NRP ranking_df shape: {ranking_df.shape}")
    ...
    # Combine data (similar to SDP version)
    combined_dfs = []
    ...
    if combined_dfs:
        ...
        print(f"NRP All combined data shape: {all_combined_df.shape}")
        ...
        print(f"NRP Final data shape after column selection: {processed_data.shape}")
        ...
        output_filename = 'processed_data_nrp.csv'
        output_path = os.path.join(output_folder, output_filename)
        processed_data.to_csv(output_path, index=False)
        print(f"NRP Final processed data saved to: {output_path}")
        ...
        print("\n" + "=" * 60)
        print("NRP Data processing pipeline with external ranking completed")
        print("=" * 60)

        return processed_data
    else:
        print("No valid NRP data combinations generated")
        return None
...
if __name__ == "__main__":
    ...
    ranking_csv_path = '../../../Results/Predict-raw-data/Ranking/non_ft_modes_ranking_nrp.csv'
