...
MAXIMIZATION_DATASETS_FT = ["workflow_1", "workflow_2", "workflow_3", "workflow_4", "workflow_5", "workflow_6", "workflow_7", "workflow_8",
    "workflow_9", "workflow_10"]

ALGORITHM_DIR_MAP = {
    'NSGA2': os.path.abspath('../../../../Results/RQ1-raw-data/WSC')
}

try:
    sk = importr('ScottKnottESD')
except Exception as e:
    print("[Error] Failed to import R package ScottKnottESD!")
    print("Please install in R console: install.packages('ScottKnottESD')")
...
def get_ft_from_nsga2(file_path):
    try:
        ...
        ft_match = re.search(r"Best Solution: latency=(-?\d+\.?\d*)", target_line)
        if ft_match:
            return float(ft_match.group(1))
        else:
            ft_match = re.search(r"Best Solution: throughput=(-?\d+\.?\d*)", target_line)
            if ft_match:
                return float(ft_match.group(1))
        return None
    except Exception as e:
        print(f"[Error] Reading NSGA2 file {file_path}: {e}")
        return None
...
def read_non_ft_best_nsga2_data(selected_datasets, start_seed, end_seed, output_path=None):
    ...
    if not os.path.exists(nsga2_dir):
        print(f"[Error] NSGA2 results directory does not exist: {nsga2_dir}")
    ...
    for file in os.listdir(nsga2_dir):
        ...
        nsga2_pattern = core_no_suffix.split('_')
        if not nsga2_pattern:
            continue

        dataset_name = nsga2_pattern[0] + '_' + nsga2_pattern[1]
        seed = int(nsga2_pattern[2])
        mode = nsga2_pattern[3]

        if (dataset_name not in selected_datasets or
        ...
        if dataset_name in MAXIMIZATION_DATASETS_FT and is_reverse:
            processed_ft = -raw_ft
        else:
            processed_ft = raw_ft

        all_data_rows.append({
            'Dataset Name': f"{dataset_name}_reverse" if is_reverse else dataset_name,
    ...
    for dataset_name in all_data_df['Dataset Name'].unique():
        dataset_data = all_data_df[all_data_df['Dataset Name'] == dataset_name]
        ...
        try:
            ...
            ranking_results_by_dataset[dataset_name] = pd.DataFrame(dataset_ranking_results)
            best_mode = all_modes_ranking[0]['mode']
            best_modes_by_dataset[dataset_name] = best_mode
    ...
    for dataset_name in best_modes_by_dataset.keys():
        dataset_data = all_data_df[all_data_df['Dataset Name'] == dataset_name]
        best_mode = best_modes_by_dataset[dataset_name]
        ...
        for seed in dataset_data['Random Seed'].unique():
            ...
            if not seed_data.empty:
                ...
                if not pd.isna(median_val):
                    default_row = {
                        'Dataset Name': dataset_name,
                        'Original Dataset': dataset_data['Original Dataset'].iloc[0] if not dataset_data.empty else dataset_name.replace('_reverse', ''),
                        'Random Seed': seed,
...
def main():
    config = {
        ...
        'ranking_filename': 'non_ft_modes_ranking_wsc.csv',
    }
    selected_datasets = ["workflow_1", "workflow_2", "workflow_3", "workflow_4", "workflow_5", "workflow_6",
                         "workflow_7", "workflow_8", "workflow_9", "workflow_10"]
    config['selected_datasets'] = selected_datasets
