...
def calculate_dominance_features(decision_vars, objective_values):
    if len(decision_vars) < 2:
        return {'median_dominance': 0, 'max_dominance': 0}
    all_objectives = np.array(objective_values)
    dominance_counts = []
    for i in range(len(decision_vars)):
        current_obj = all_objectives[i]
...
def print_and_save_statistics(sampled_data, sampled_dict, r_pareto_count, total_points_count, fig_num, x, y,
    ...
    pareto_ratio = total_points_count.get('pareto', 0) / total_points if total_points > 0 else 0
    ...
    row = [
        ...
        float(dominance_change_frequency_stat) if isinstance(dominance_change_frequency_stat, (int, float)) else dominance_change_frequency_stat,
        grid_density_var,
        hypervolume,
        kendalltau_corr,
        level_count,
