...
ALGORITHM_DIR_MAP = {
    'NSGA2': os.path.abspath('../../../../Results/RQ1-raw-data/NAS')
}
...
def read_non_ft_best_nsga2_data(selected_datasets, start_seed, end_seed, output_path=None):
    ...
    for file in os.listdir(nsga2_dir):
        ...
        if not nsga2_pattern or len(nsga2_pattern) < 4:
            continue
...
def main():
    config = {
        ...
        'ranking_filename': 'non_ft_modes_ranking_nas.csv',
    }

    selected_datasets = []
    problem_types = ['c10mop', 'citysegmop', 'in1kmop']
    c10mop_ids = [1, 3, 5, 8, 10, 11, 12, 13]
    citysegmop_ids = [3]
    in1kmop_ids = [1, 4, 7]

    for idx, problem_type in enumerate(problem_types):
        if idx == 0:
            for pid in c10mop_ids:
                num_combos = len(c10_search_space_configs[pid])
                for num in range(num_combos):
                    selected_datasets.append(f"{problem_type}{pid}_{num}")
        elif idx == 1:
            for pid in citysegmop_ids:
                num_combos = len(cityseg_search_space_configs[pid])
                for num in range(num_combos):
                    selected_datasets.append(f"{problem_type}{pid}_{num}")
        elif idx == 2:
            for pid in in1kmop_ids:
                num_combos = len(in1k_search_space_configs[pid])
                for num in range(num_combos):
                    selected_datasets.append(f"{problem_type}{pid}_{num}")
