...
migration_rules = ['ant-1.7', 'camel-1.6', 'ivy-2.0', 'jedit-4.0', 'lucene-2.4',
                'poi-3.0', 'synapse-1.2', 'velocity-1.6', 'xalan-2.6', 'xerces-1.4'
]
input_folders = ["J48","KNN","LR","NB"]
selected_datasets = []
...
ALGORITHM_DIR_MAP = {
    'NSGA2': os.path.abspath('../../../../Results/RQ1-raw-data/SDP')
}
...
def get_ft_from_nsga2(file_path):
    try:
        ...
        if not lines:
            return None
        target_line = lines[-3] if len(lines) >= 2 else lines[-1]
        ft_match = re.search(r"Best Solution: 'auc': (-?\d+\.?\d*)", target_line)
        if ft_match:
...
def main():
    config = {
        ...
        'ranking_filename': 'non_ft_modes_ranking_sdp.csv',
    }
