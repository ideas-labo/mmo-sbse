...
class ExactHammingIndex:
    ...
    def search(self, query, k, max_candidates=1000):
        if not self.data:
            return [], []
        k = min(k, get_adaptive_k(self.dimension))
        ...
        nearest_indices = [candidate_indices[i] for i in sorted_idx]
        nearest_distances = [distances[i] for i in sorted_idx]
...
def find_local_efficient_points(decision_vars, objective_values, sampled_data, random_seed=None):
    ...
    for i, var in enumerate(dv):
        ...
        for n in neighbors:
            ...
            if j is not None:
                other_obj = ov[j]
                if (other_obj[0] <= cur_obj[0] and other_obj[1] < cur_obj[1]) or (other_obj[0] < cur_obj[0] and other_obj[1] <= cur_obj[1]):
                    dominated = True
                    break
...
def calculate_local_to_global_ratio(decision_vars, objective_values, sampled_data, random_seed=None):
    ...
    return {'local_as_global_ratio': float(len(inter) / len(lidx)) if len(lidx) > 0 else 0.0,
            'num_global': len(gidx), 'num_local': len(lidx), 'num_intersection': len(inter),
...
def print_and_save_statistics(sampled_data, sampled_dict, r_pareto_count, total_points_count,
    ...
    if len(x) >= 2 and len(y) >= 2:
        ...
        pareto_idx = find_global_pareto_points(decision_vars, objective_values)
        pareto_decisions = decision_vars[pareto_idx] if len(pareto_idx) > 0 else np.array([])
        if len(pareto_decisions) > 1:
            dmat = pairwise_distances(np.array(pareto_decisions), metric='hamming')
...
def plot_figure1(random_seed=42, center_type='random', sampling_method='', sample_size='',
    ...
    for point in r0_points:
        ...
        angle = np.arctan2(point[1] - center_point[1], point[0] - center_point[0]) % (2 * np.pi)
        for i in range(8):
    ...
    print_and_save_statistics(sampled_data, sampled_dict, r_pareto_count, total_points_count,
                              ...
                              mode, unique_id, sample_type, dominance_change_frequency, reverse)
