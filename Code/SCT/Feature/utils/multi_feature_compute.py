...
def generate_neighbor_solutions(solution, sampled_data, random_seed=None):
    ...
    distances = []
    for candidate in sampled_set:
        if candidate == solution:
            continue
        distance = sum(1 for x, y in zip(solution, candidate) if x != y)
    ...
    neighbors = [candidate for candidate, _ in distances[:k]]
    return neighbors
...
def find_local_efficient_points(decision_vars, objective_values, sampled_data, random_seed=None):
    ...
    local_efficient = []
    for i, cur in enumerate(decision_vars):
        cur_t = tuple(cur)
        cur_obj = objective_values[i]
        neighbors = generate_neighbor_solutions(cur_t, sampled_set, random_seed=random_seed)
        dominated = False
        for n in neighbors:
            n_obj = var_to_obj.get(n)
            if n_obj is None:
                continue
            if (n_obj[0] <= cur_obj[0] and n_obj[1] < cur_obj[1]) or \
               (n_obj[0] < cur_obj[0] and n_obj[1] <= cur_obj[1]):
                dominated = True
                break
        if not dominated:
...
def calculate_grid_features(decision_vars, objective_values, is_pareto_list, step=0.2, feature_range=(-2, 2)):
    ...
    count_values = np.array(list(grid_counts.values())) if len(grid_counts) > 0 else np.array([0.0])
    ...
    return {'grid_density_var': density_var, 'pareto_grid_ratio': pareto_ratio,
            'adjacent_grid_hamming_mean': adj_mean}
    ...
    for var in decision_vars:
        ...
        for n in neighbors:
            ...
            if (n_obj[0] <= cur_obj[0] and n_obj[1] < cur_obj[1]) or (n_obj[0] < cur_obj[0] and n_obj[1] <= cur_obj[1]):
                dom_count += 1
        total_prop += (dom_count / len(neighbors))
        valid += 1
    return float(total_prop / valid) if valid > 0 else 0.0
...
def calculate_local_to_global_ratio(decision_vars, objective_values, sampled_data, random_seed=None):
    ...
    inter = global_set.intersection(local_set)
    return {'local_as_global_ratio': float(len(inter) / len(local_set)) if len(local_set) > 0 else 0.0,
            'num_global': len(global_set), 'num_local': len(local_set), 'num_intersection': len(inter),
...
def print_and_save_statistics(sampled_data, sampled_dict, r_pareto_count, total_points_count, fig_num, x, y,
                              random_seed,
                              sampling_method, sample_size, dataset_name, mode, unique_id, sample_type,
                              dominance_change_frequency, supp_proportion, reverse):
...
def plot_figure1(random_seed=42, center_type='random', sampling_method='', sample_size='',
                 dataset_name='', mode='', unique_id='', sample_type='', sampled_dict=None, reverse=False):
    ...
    supported_count = sum(1 for p in pareto_points if is_supported_solution(p, pareto_points))
    supp_proportion = supported_count / len(pareto_points) if len(pareto_points) > 0 else 0.0

    dominance_change_frequency = calculate_dominance_change_frequency(sampled_dict)
...
def plot_figure2(random_seed=42, center_type='random', sampling_method='', sample_size='', dataset_name='',
    ...
    try:
        ...
        print_and_save_statistics(sampled_data, sampled_dict, r_pareto_count, total_points_count, 2, x, y,
                                  ...
                                  unique_id, sample_type, dominance_change_frequency, supp_proportion, reverse)
    except Exception as e:
        print(f"Error processing Figure 2: {e}")


def is_supported_solution(solution, pareto_front):
