...
MAXIMIZATION_DATASETS_FT = [
    'storm_wc', 'storm_rs', 'storm_sol', 'dnn_dsr',
    'dnn_coffee', 'dnn_adiac', 'trimesh', 'dnn_sa', 'x264'
]
ALGORITHM_DIR_MAP = {
    'NSGA2': os.path.abspath('../../../../Results/RQ1-raw-data/SCT')
}
...
def read_non_ft_best_nsga2_data(selected_datasets, start_seed, end_seed, output_path=None):
    ...
    for file in os.listdir(nsga2_dir):
        ...
        nsga2_pattern = re.match(r'([a-zA-Z0-9_-]+)-(\d+)_([a-zA-Z0-9]+)', core_no_suffix)
        if not nsga2_pattern:
            continue

        dataset_name = nsga2_pattern.group(1)
        seed = int(nsga2_pattern.group(2))
        mode = nsga2_pattern.group(3)
...
def main():
    config = {
        ...
        'selected_datasets': ['dnn_adiac', 'dnn_coffee', 'dnn_dsr', 'dnn_sa',
                              'llvm', 'lrzip', 'mariadb', 'mongodb', 'storm_rs',
                              'storm_wc', 'trimesh', 'vp9', 'x264'],
        'output_folder': os.path.abspath('../../../../Results/Predict-raw-data/Ranking'),
        'ranking_filename': 'non_ft_modes_ranking_sct.csv',
    }
